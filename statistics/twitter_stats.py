...
def calculate_top_words(result_path,topn):
    """ get top word
    """
    stopword = pd.read_csv(dir+"/utils/dictionary/twitter_stopwords.txt",index_col=0).iloc[:,0].values
...
def pre_opening_analysis(keyword_list,flr_thres):
    for key_word in keyword_list:
        file_name = f'{dir}/utils/data/senti_results/{key_word}/{key_word}_{flr_thres}.csv'
        ...
        sresult.to_csv(f"{dir}/utils/data/preopen/{today}/{key_word}.csv")
